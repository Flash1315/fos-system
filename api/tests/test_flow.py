...
def test_occurred_at_affects_spendings_cutoff(client):
    ...
    blocked = client.post(
        f"/records/{late.json()['id']}/decide",
        headers=h,
        json={"approve": True},
    )
    assert blocked.status_code == 400
    assert "settled cycle" in blocked.json()["detail"].lower()
    forced = client.post(
        f"/records/{late.json()['id']}/decide",
        headers=h,
        json={"approve": True, "allow_closed_cycle": True},
    )
...
def test_overpayment_rejects_client_value(client):
    ...
    ok = client.post(
        ...
        json={
            ...
            "amount": 12000,
            "payment_method": "cash",
            "note": "server computes overpay",
        },
    )
    assert ok.status_code == 200, ok.text
    assert ok.json()["overpayment"] == 2000
...
def test_billing_and_money_numeric(client):
    ...
    assert health.json()["version"] == "0.8.230"
...
def test_manager_invite_role_comment_cap_and_odo_bounds(client):
    ...
    # Comment append truncates older text instead of blocking lifecycle/notes
    pending = client.post(
        "/records",
        headers=h,
        json={
    ...
    assert pending.status_code == 200, pending.text
    rid = pending.json()["id"]
    note = "this note is long enough to overflow the comment budget"
    huge = client.post(
    ...
    assert note in huge.json()["comment"]
    assert len(huge.json()["comment"]) <= 4000
    assert "…" in huge.json()["comment"]
...
def test_health_controls_before_collapse_and_export_names(client, monkeypatch):
    ...
    monkeypatch.setattr(settings, "rate_limit_enabled", True)
    try:
        # /health/live is intentionally not rate-limited (orchestrator probes).
        # Public login form still is.
        limited = None
        for _ in range(80):
            limited = client.post(
                "/auth/login-form",
                data={"username": "nope@x.com@@missing-org", "password": "x"},
            )
            if limited.status_code == 429:
                break
        assert limited is not None and limited.status_code == 429
...
def test_preauth_429_body_limit_purpose_and_settings(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(settings, "database_url", "postgresql+psycopg2://fos:fos@localhost:5432/fos")
    monkeypatch.setattr(settings, "cors_origins", "https://app.example.com")
    monkeypatch.setattr(settings, "metrics_token", "ci-metrics-token-32chars-minxxxx")
...
def test_billing_freeze_mutations_and_media_type(client):
    ...
    owner = _register(client, "flow-0742", "v0742-owner@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    rec = client.post(
        "/records",
        headers=h,
    ...
    db = SessionLocal()
    try:
        org = db.query(Organization).filter(Organization.slug == "flow-0742").one()
        org.billing_status = "canceled"
        db.commit()
    finally:
        db.close()
    ...
    assert "canceled" in decide.json()["detail"].lower()

    inv = client.post(
        "/orgs/invite",
        headers=h,
        json={
            "email": "blocked-0742@example.com",
            "full_name": "Blocked",
            "role": "employee",
        },
    )
    ...
    # Restore billing to finish invitee JWT defense check on a fresh org
    db = SessionLocal()
    try:
        org = db.query(Organization).filter(Organization.slug == "flow-0742").one()
    ...
    inv2 = client.post(
        ...
        headers=h,
        json={
            "email": "invitee-0742@example.com",
            "full_name": "Invitee",
            "role": "employee",
        },
    )
    ...
    db = SessionLocal()
    try:
        u = db.query(User).filter(User.email == "invitee-0742@example.com").one()
...
def test_past_due_readonly_overpay_forbid_and_health_backend(client, monkeypatch):
    """v0.7.45: past_due freeze, payout forbids client overpayment, health uses effective media."""
    ...
    monkeypatch.setattr(settings, "media_backend", "s3")
    monkeypatch.setattr(settings, "s3_bucket", "")
    assert media_backend() == "local"
    monkeypatch.setattr(settings, "media_backend", "local")

    owner = _register(client, "flow-0745", "v0745-owner@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    pending = client.post(
        "/records",
        headers=h,
    ...
    db = SessionLocal()
    try:
        org = db.query(Organization).filter(Organization.slug == "flow-0745").one()
    ...
    assert blocked.status_code == 403
    assert "past due" in blocked.json()["detail"].lower()
    ...
    login = client.post(
        "/auth/login",
        json={
            "email": "v0745-owner@example.com",
            "password": "secret12",
            "organization_slug": "flow-0745",
        },
    )
    assert login.status_code == 200

    from app.config import settings as _settings
    from app.services.notify import _TELEGRAM_TIMEOUT_SEC

    assert float(_settings.smtp_timeout_seconds) == 5
    assert _TELEGRAM_TIMEOUT_SEC == 5


def test_cancel_creator_only_append_truncate_and_xff(client, monkeypatch):
    """v0.7.46: creator-only cancel, comment truncate, hardened XFF, version bump."""
    from app.version import APP_VERSION
    ...
    assert inv.status_code == 200, inv.text
    eh = {
        "Authorization": f"Bearer {client.post('/auth/login', json={'email': 'v0746-emp@example.com', 'password': 'secret12', 'organization_slug': 'flow-0746'}).json()['access_token']}"
    }
    rec = client.post(
        "/records",
        headers=eh,
    ...
    blocked = client.delete(f"/records/{rid}", headers=h)
    assert blocked.status_code == 403
    assert "creator" in blocked.json()["detail"].lower()

    rejected = client.post(
        f"/records/{rid}/decide",
        headers=h,
        json={"approve": False, "note": "missing receipt"},
    )
    assert rejected.status_code == 200, rejected.text
    assert rejected.json()["status"] == "rejected"

    from app.routers.records import _COMMENT_MAX, _append_text

    huge = "x" * _COMMENT_MAX
    out = _append_text(huge, "[cancelled]")
    assert out.endswith("[cancelled]")
    assert len(out) <= _COMMENT_MAX
    assert "…" in out
    ...
    try:
        # Immediate peer is trusted proxy; rightmost untrusted hop is the client.
        assert (
            client_ip(_Req("10.0.0.1", "198.51.100.7, 10.0.0.2, 10.0.0.1"))
            == "198.51.100.7"
        )
        # Malformed hop falls back to peer.
        assert client_ip(_Req("10.0.0.1", "not-an-ip, 10.0.0.1")) == "10.0.0.1"
    finally:
        monkeypatch.setattr(settings, "trust_x_forwarded_for", False)
        monkeypatch.setattr(settings, "trusted_proxy_cidrs", "")


def test_csv_settled_amount_currency_and_startup_pairs(client, monkeypatch):
    """v0.7.47: CSV settled_amount column + currency; ALGORITHM/S3 key pairing."""
    from app.version import APP_VERSION
    import csv
    ...
    assert sreq.status_code == 200, sreq.text
    approved = client.post(f"/payouts/requests/{sreq.json()['id']}/approve", headers=h)
    assert approved.status_code == 200, approved.text

    csv_res = client.get("/reports/export.csv?days=30", headers=h)
    assert csv_res.status_code == 200, csv_res.text
    assert csv_res.text.startswith("\ufeff")
    csv_body = csv_res.text.lstrip("\ufeff")
    rows = list(csv.DictReader(StringIO(csv_body)))
    header = csv_body.splitlines()[0]
    assert "settled_amount" in header
    payouts = [r for r in rows if r["type"] == "payout"]
    assert payouts
    assert payouts[0].get("settled_amount", "") == ""
    adjs = [r for r in rows if r["type"] == "adjustment"]
    assert adjs and adjs[0]["currency"] == "IDR"
    srows = [r for r in rows if r["type"] == "settlement_request"]
    assert srows
    assert srows[0]["currency"] == "IDR"
    assert float(srows[0]["settled_amount"] or 0) > 0
    assert srows[0]["overpayment"] == ""

    from app.main import _validate_runtime_settings
    from app.config import settings
    from app.services.storage import load_photo

    monkeypatch.setattr(settings, "algorithm", "")
    try:
        raised = False
        try:
            _validate_runtime_settings()
    ...
    monkeypatch.setattr(settings, "s3_access_key", "AKIA")
    ...
    try:
        ...
        try:
            ...
            raised = True
            assert "S3_ACCESS_KEY" in str(exc)
        assert raised
    finally:
        monkeypatch.setattr(settings, "s3_access_key", "")
        monkeypatch.setattr(settings, "s3_secret_key", "")

    data, _ctype, err = load_photo(1, "0123456789abcdef0123456789abcdef.jpg")
    assert data is None and err == "not_found"


def test_rate_limit_prod_gate_limiter_cap_and_cancel_replay_auth(client, monkeypatch):
    """v0.7.78: prod rate-limit gate, limiter key cap, cancel idem replay auth."""
    from app.version import APP_VERSION
    ...
    lim = rl.FixedWindowLimiter()
    monkeypatch.setattr(rl, "_MAX_KEYS", 20)
    monkeypatch.setattr(rl, "_PRUNE_AFTER_KEYS", 5)
    for i in range(40):
        lim.check(f"k{i}", limit=100, window_sec=60)
    assert len(lim._hits) <= 20

    owner = _register(client, "flow-0748", "v0748-owner@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    inv = client.post(
        "/orgs/invite",
        headers=h,
        json={
            "email": "v0748-emp@example.com",
            "full_name": "Emp",
            "role": "employee",
            "password": "secret12",
            "password_confirm": "secret12",
    ...
    mh = {"Authorization": f"Bearer {mgr_login.json()['access_token']}"}
    income = client.post(
        "/records",
        headers=eh,
        json={
    ...
    sreq = client.post(
        "/payouts/requests",
        headers=eh,
        json={"kind": "income_handover", "amount": 1000, "note": "handover"},
    )
    assert sreq.status_code == 200, sreq.text
    rid = sreq.json()["id"]
    cancel_headers = {**mh, "Idempotency-Key": "cancel-0748-mgr"}
    first = client.post(
        f"/payouts/requests/{rid}/cancel",
        headers=cancel_headers,
        json={"note": "mgr cancel"},
    )
    assert first.status_code == 200, first.text
    # Demote former manager — replay of cancel must re-check role.
    demote = client.post(
        f"/orgs/members/{mgr_id}/role",
        headers=h,
        json={"role": "employee"},
    )
    assert demote.status_code == 200, demote.text
    # Old JWT still has manager role claim until bump — role change bumps token_version.
    assert client.get("/auth/me", headers=mh).status_code == 401
    mgr_login2 = client.post(
        "/auth/login",
        json={
            "email": "v0748-mgr@example.com",
            "password": "secret12",
    ...
    mh2 = {
        "Authorization": f"Bearer {mgr_login2.json()['access_token']}",
        "Idempotency-Key": "cancel-0748-mgr",
    }
    blocked = client.post(
        f"/payouts/requests/{rid}/cancel",
        headers=mh2,
...
def test_login_form_validation_and_docker_pack_markers(client):
    """v0.7.78: login-form ValidationError → 422; deploy pack markers."""
    from app.version import APP_VERSION
    from pathlib import Path
    ...
    bad_email = client.post(
        "/auth/login-form",
        data={"username": "not-an-email@@acme", "password": "secret12"},
    )
    assert bad_email.status_code == 422, bad_email.text

    bad_slug = client.post(
        "/auth/login-form",
        data={"username": "ok@example.com@@BAD_SLUG", "password": "secret12"},
    )
    assert bad_slug.status_code == 422, bad_slug.text

    missing_sep = client.post(
        "/auth/login-form",
        data={"username": "ok@example.com", "password": "secret12"},
    )
    assert missing_sep.status_code == 400

    api_root = Path(__file__).resolve().parents[1]
    dockerfile = (api_root / "Dockerfile").read_text(encoding="utf-8")
    assert "COPY alembic.ini" in dockerfile
    assert "COPY alembic" in dockerfile
    assert "USER fos" in dockerfile
    assert (api_root / ".dockerignore").is_file()
    env_ex = (api_root / ".env.example").read_text(encoding="utf-8")
    assert "SMTP_HOST" in env_ex
    assert "PUBLIC_APP_URL" in env_ex
    assert "TELEGRAM_BOT_TOKEN" in env_ex
    assert "BILLING_PLAN_SWITCH" in env_ex


def test_freeze_idem_replay_accept_invite_and_local_media_oserror(client, monkeypatch):
    """v0.7.78: idem replay under freeze, accept-invite OK, local media OSError → 503."""
    from app.version import APP_VERSION
    from pathlib import Path
    ...
    # Invite before freeze; accept during freeze
    inv = client.post(
        "/orgs/invite",
        headers=h,
        json={
            "email": "v0750-emp@example.com",
            "full_name": "Emp",
            "role": "employee",
        },
    )
    ...
    created = client.post(
        "/records",
        headers={**h, "Idempotency-Key": "rec-create-0750"},
        json={
            "kind": "expense",
            "amount": 12.5,
            "category": "Other",
    ...
    with SessionLocal() as db:
        org = db.query(Organization).filter(Organization.slug == "flow-0750").one()
        org.billing_status = "past_due"
        db.commit()
    ...
    blocked = client.post(
        "/records",
        headers={**h, "Idempotency-Key": "rec-create-0750-new"},
        json={
            "kind": "expense",
            "amount": 1,
            "category": "Other",
    ...
    # Same key replays stored create despite freeze
    replay = client.post(
        "/records",
        headers={**h, "Idempotency-Key": "rec-create-0750"},
        json={
    ...
    # Cancel pending still OK
    cancel = client.delete(
        f"/records/{rid}",
        headers={**h, "Idempotency-Key": "rec-cancel-0750"},
    )
    ...
    # Local media OSError → unavailable / 503
    def boom(*_a, **_k):
        raise OSError("permission denied")

    monkeypatch.setattr(Path, "is_file", lambda self: True)
    monkeypatch.setattr(Path, "read_bytes", boom)
    data, _ctype, err = storage.load_photo(1, "0123456789abcdef0123456789abcdef.jpg")
    assert data is None and err == "unavailable"

    api_root = Path(__file__).resolve().parents[1]
    dockerfile = (api_root / "Dockerfile").read_text(encoding="utf-8")
    assert "HEALTHCHECK" in dockerfile
    reqs = (api_root / "requirements.txt").read_text(encoding="utf-8")
    assert "pytest" not in reqs
    assert (api_root / "requirements-dev.txt").is_file()
    compose = (api_root.parent / "docker-compose.yml").read_text(encoding="utf-8")
    assert "start_period" in compose
    assert "127.0.0.1:5432:5432" in compose


def test_prod_refuses_sqlite_and_deploy_pack_markers(client, monkeypatch):
    """v0.7.78: production refuses SQLite; pool bounds; deploy docs/compose example."""
    from pathlib import Path
    from app.version import APP_VERSION
    from app.main import _validate_runtime_settings
    from app.config import settings
    from app import alembic_runner
    ...
    monkeypatch.setattr(settings, "cors_origins", "https://app.example.com")
    monkeypatch.setattr(settings, "metrics_token", "ci-metrics-token-32chars-minxxxx")
    monkeypatch.setattr(settings, "database_url", "sqlite:///./nope.db")
    try:
        raised = False
        try:
            _validate_runtime_settings()
            ...
            raised = True
            assert "PostgreSQL" in str(exc) or "sqlite" in str(exc).lower()
        assert raised
    finally:
        monkeypatch.setattr(settings, "database_url", "sqlite:///./fos.db")
        monkeypatch.setattr(settings, "environment", "development")
        monkeypatch.setattr(settings, "secret_key", "test-secret")
    ...
    try:
        ...
        try:
            ...
            raised = True
            assert "DB_POOL_SIZE" in str(exc)
        assert raised
    finally:
    ...
    # Prod alembic path raises on failure
    monkeypatch.setattr(settings, "environment", "production")

    def boom():
        raise RuntimeError("forced alembic failure")

    # Patch command.upgrade indirectly by making Config raise... easier: patch runner internals
    import alembic.command as ac

    def fail_upgrade(cfg, rev):
        raise RuntimeError("forced alembic failure")

    monkeypatch.setattr(ac, "upgrade", fail_upgrade)
    try:
        raised = False
        try:
            alembic_runner.run_alembic_upgrade()
        except RuntimeError as exc:
            raised = True
            assert "Alembic upgrade failed" in str(exc)
        assert raised
    finally:
        monkeypatch.setattr(settings, "environment", "development")
    ...
    assert (root / "docker-compose.prod.example.yml").is_file()
    env_ex = (root / "api" / ".env.example").read_text(encoding="utf-8")
    assert "DB_POOL_SIZE" in env_ex
    assert "DB_SSLMODE" in env_ex


def test_eas_pack_markers(client):
    """v0.7.78: EAS docs + preview/production env gate markers."""
    from pathlib import Path
    from app.version import APP_VERSION
    ...
    eas = (root / "mobile" / "eas.json").read_text(encoding="utf-8")
    assert "EXPO_PUBLIC_APP_ENV" in eas
    assert '"preview"' in eas
    cfg = (root / "mobile" / "app.config.js").read_text(encoding="utf-8")
    assert "preview" in cfg and "https://" in cfg
    pkg = (root / "mobile" / "package.json").read_text(encoding="utf-8")
    assert "config:check" in pkg
    assert "build:prod" in pkg


def test_redis_limiter_fallback_and_hot_indexes(client, monkeypatch):
    """v0.7.78: Redis limiter with fallback; hot-index migration markers."""
    from pathlib import Path
    from app.version import APP_VERSION
    from app.config import settings
    from app.services import rate_limit as rl
    ...
    class BoomRedis:
        def check(self, key, *, limit, window_sec):
            raise RuntimeError("redis down")

        def reset(self):
            pass

    monkeypatch.setattr(rl._limiter, "_redis", BoomRedis())
    monkeypatch.setattr(rl._limiter, "_redis_failed", False)
    rl._limiter._memory.reset()
    assert rl._limiter.check("fb-a", limit=2, window_sec=60)[0] is True
    assert rl._limiter.check("fb-a", limit=2, window_sec=60)[0] is True
    ok, retry = rl._limiter.check("fb-a", limit=2, window_sec=60)
    ...
    fake = CountingRedis()
    monkeypatch.setattr(rl._limiter, "_redis", fake)
    monkeypatch.setattr(rl._limiter, "_redis_failed", False)
    assert rl._limiter.check("rd-a", limit=1, window_sec=60)[0] is True
    ok, retry = rl._limiter.check("rd-a", limit=1, window_sec=60)
    assert ok is False and retry == 3
    assert fake.n == 2

    root = Path(__file__).resolve().parents[1]
    mig = root / "alembic" / "versions" / "20260728_0002_hot_indexes.py"
    assert mig.is_file()
    body = mig.read_text(encoding="utf-8")
    assert "ix_money_records_org_status_void_created" in body
    assert "ix_payouts_org_user_kind_void_created" in body
    assert "ix_idempotency_keys_created_at" in body
    models = (root / "app" / "models.py").read_text(encoding="utf-8")
    assert "ix_settlement_requests_org_user_status" in models
    reqs = (root / "requirements.txt").read_text(encoding="utf-8")
    assert "redis" in reqs


def test_ops_cors_health_metrics_and_alembic_first(client, monkeypatch):
    """v0.7.78: CORS hard gate, live/ready/metrics, prod skips create_all."""
    from pathlib import Path
    from app.version import APP_VERSION
    from app.main import _validate_runtime_settings
    from app.config import settings
    ...
    assert "fos_http_requests_total" in metrics.text
    assert "fos_up" in metrics.text

    metrics_tok = "secret-metricsxxxxxxxxxxxxxxxxxx"
    monkeypatch.setattr(settings, "metrics_token", metrics_tok)
    denied = client.get("/metrics")
    assert denied.status_code == 401
    okm = client.get("/metrics", headers={"X-Metrics-Token": metrics_tok})
    assert okm.status_code == 200
    ...
    monkeypatch.setattr(settings, "rate_limit_enabled", True)
    monkeypatch.setattr(
        settings, "database_url", "postgresql+psycopg2://fos:fos@localhost:5432/fos"
    )
    monkeypatch.setattr(settings, "cors_origins", "*")
    try:
        raised = False
    ...
    # Marker: prod boot skips create_all
    main_src = (Path(__file__).resolve().parents[1] / "app" / "main.py").read_text(
        encoding="utf-8"
    )
    assert "if not _IS_PROD:" in main_src
    assert "create_all" in main_src
    assert "/health/live" in main_src
    dockerfile = (Path(__file__).resolve().parents[1] / "Dockerfile").read_text(
        encoding="utf-8"
    )
    assert "health/live" in dockerfile
    assert "RATE_LIMIT_REDIS_URL" in dockerfile or "workers" in dockerfile.lower()
    gi = (Path(__file__).resolve().parents[2] / ".gitignore").read_text(encoding="utf-8")
    assert "google-services.json" in gi
    assert "mobile/.env" in gi


def test_closed_cycle_gate_photo_idem_and_comment_status(client):
    """v0.7.78: closed-cycle approve gate, photo idempotency, comment status."""
    from pathlib import Path
    from app.version import APP_VERSION
    ...
    rid = late.json()["id"]
    blocked = client.post(
        f"/records/{rid}/decide", headers=h, json={"approve": True}
    )
    assert blocked.status_code == 400
    ...
    up1 = client.post(
        ...
        files={"file": ("r.jpg", usable, "image/jpeg")},
    )
    assert up1.status_code == 200, up1.text
    url1 = up1.json()["photo_url"]
    up2 = client.post(
        "/media/photo",
        headers={**h, "Idempotency-Key": "photo-0755"},
        files={"file": ("r.jpg", usable, "image/jpeg")},
    ...
    assert up2.json()["photo_url"] == url1
    ...
    jid = rej.json()["id"]
    client.post(
        f"/records/{jid}/decide",
        headers=h,
        json={"approve": False, "note": "nope"},
    )
    cmt = client.post(
        f"/records/{jid}/comment",
        headers=h,
        json={"note": "after reject"},
    )
    assert cmt.status_code == 400

    media_src = (
        Path(__file__).resolve().parents[1] / "app" / "routers" / "media.py"
    ).read_text(encoding="utf-8")
    assert 'scope="media.photo"' in media_src


def test_invite_reset_token_visibility_retry_and_closed_cycle_client(client, monkeypatch):
    """v0.7.56–0.7.78: issuer always gets token; client markers; metrics token gate."""
    from pathlib import Path
    from app.version import APP_VERSION
    from app.main import _validate_runtime_settings
    from app.config import settings
    ...
    monkeypatch.setattr(
        "app.services.email.send_reset_email",
        lambda **_k: True,
    )
    # Accept first so reset-token path applies to settled member... actually must_set still true
    # Issue reset while invite active → 409 without force
    r409 = client.post(f"/orgs/members/{mid}/reset-token", headers=h)
    assert r409.status_code == 409
    assert "force=true" in r409.json()["detail"]
    assert "share the previous" not in r409.json()["detail"].lower()

    reset = client.post(f"/orgs/members/{mid}/reset-token?force=true", headers=h)
    assert reset.status_code == 200, reset.text
    assert reset.json().get("invite_token")
    ...
    assert "shouldSoftRetry" in api_ts
    assert "allowClosedCycle" in api_ts
    assert "parseRetryAfterSec" in api_ts
    assert "probeApiLive" in api_ts
    approve = (root / "mobile" / "src" / "screens" / "ApproveScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "allowClosedCycle" in approve
    assert "skipped_closed_cycle" in approve
    app = (root / "mobile" / "App.tsx").read_text(encoding="utf-8")
    assert "probeApiLive" in app and "AppState" in app


def test_soft_retry_resume_content_hash_and_prod_metrics_compose(client):
    """v0.7.59–0.7.78: safe soft-retry, resume bus, content-hash media, prod metrics."""
    import hashlib
    from pathlib import Path
    ...
    root = Path(__file__).resolve().parents[2]
    api_ts = (root / "mobile" / "src" / "api.ts").read_text(encoding="utf-8")
    assert "safeRetry" in api_ts
    assert 'isSafeSoftRetry' in api_ts or 'method === "GET" || hasIdem' in api_ts
    assert "onResumeRefresh" in api_ts
    assert "notifyResumeRefresh" in api_ts
    assert "cachedMediaToken" in api_ts
    assert "14 * 60 * 1000" in api_ts

    create = (root / "mobile" / "src" / "screens" / "CreateScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "allow_closed_cycle" in create
    approve = (root / "mobile" / "src" / "screens" / "ApproveScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "skipped_closed_cycle" in approve
    # reject-all path also surfaces settled-period skips
    assert approve.count("skipped_closed_cycle") >= 2

    app = (root / "mobile" / "App.tsx").read_text(encoding="utf-8")
    assert "notifyResumeRefresh" in app
    home = (root / "mobile" / "src" / "screens" / "HomeScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "onResumeRefresh" in home

    storage_ts = (root / "mobile" / "src" / "storage.ts").read_text(encoding="utf-8")
    assert "WHEN_UNLOCKED_THIS_DEVICE_ONLY" in storage_ts

    media_src = (
        Path(__file__).resolve().parents[1] / "app" / "routers" / "media.py"
    ).read_text(encoding="utf-8")
    assert "content_sha[:32]" in media_src
    assert "uuid.uuid4" not in media_src
    storage_py = (
        Path(__file__).resolve().parents[1] / "app" / "services" / "storage.py"
    ).read_text(encoding="utf-8")
    assert "def delete_photo" in storage_py
    ...
    assert "METRICS_TOKEN` is **required**" in deploy or "required** in production" in deploy
    env_ex = (root / "api" / ".env.example").read_text(encoding="utf-8")
    assert "Required when ENVIRONMENT=production" in env_ex
...
def test_resume_billing_media_hygiene_and_ready_media(client):
    """v0.7.63–0.7.78: resume bus, freeze UX, cancel photo cleanup, ready media."""
    import hashlib
    from pathlib import Path

    from app.version import APP_VERSION
    from app.services.storage import load_photo, UPLOAD_ROOT
    ...
    assert ready.status_code == 200
    assert ready.json()["db"] == "ok"
    assert ready.json()["media"] in ("ok", "error")

    metrics = client.get("/metrics")
    assert metrics.status_code == 200
    assert "fos_media_up" in metrics.text

    root = Path(__file__).resolve().parents[2]
    api_ts = (root / "mobile" / "src" / "api.ts").read_text(encoding="utf-8")
    assert "isBillingReadOnly" in api_ts
    assert "BILLING_READONLY_MSG" in api_ts
    assert "onResumeRefresh" in api_ts
    assert 'isSafeSoftRetry' in api_ts or 'method === "GET" || hasIdem' in api_ts
    ...
    for name in (
        ...
        src = (root / "mobile" / "src" / "screens" / name).read_text(encoding="utf-8")
        assert "onResumeRefresh" in src, name
    ...
    for name in (
        ...
        src = (root / "mobile" / "src" / "screens" / name).read_text(encoding="utf-8")
        assert "BILLING_READONLY_MSG" in src or "isBillingReadOnly" in src, name

    detail = (root / "mobile" / "src" / "screens" / "RecordDetailScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert 'title="Cancel record"' in detail
    assert "disabled={busy}" in detail  # cancel stays enabled under freeze

    storage_py = (
        Path(__file__).resolve().parents[1] / "app" / "services" / "storage.py"
    ).read_text(encoding="utf-8")
    assert "head_object" in storage_py
    assert "def media_health" in storage_py
    assert "def delete_photo" in storage_py

    records_py = (
        Path(__file__).resolve().parents[1] / "app" / "routers" / "records.py"
    ).read_text(encoding="utf-8")
    assert "delete_photo" in records_py
    assert "{uuid}" not in records_py
    assert "sha256-prefix" in records_py

    main_py = (
        Path(__file__).resolve().parents[1] / "app" / "main.py"
    ).read_text(encoding="utf-8")
    assert "prune_expired_idem" in main_py
    assert '"media"' in main_py or "media_status" in main_py

    deploy = (root / "docs" / "DEPLOY.md").read_text(encoding="utf-8")
    assert "/health/live" in deploy and "/health/ready" in deploy
    ...
    assert rec.status_code == 200, rec.text
    rid = rec.json()["id"]
    # File exists before cancel
    org_id = int(photo_url.split("/")[3])
    filename = photo_url.rsplit("/", 1)[-1]
    data, _ct, err = load_photo(org_id, filename)
    assert err is None and data

    cancel = client.delete(f"/records/{rid}", headers=h)
    assert cancel.status_code == 200, cancel.text
    data2, _ct2, err2 = load_photo(org_id, filename)
    ...
    up2 = client.post(
        "/media/photo",
        headers=h,
        files={"file": ("d.jpg", payload2, "image/jpeg")},
    )
    assert up2.status_code == 200
    shared_url = up2.json()["photo_url"]
    r1 = client.post(
        "/records",
        headers=h,
        json={
    ...
    assert client.delete(f"/records/{r1}", headers=h).status_code == 200
    fname2 = shared_url.rsplit("/", 1)[-1]
    oid2 = int(shared_url.split("/")[3])
    data3, _ct3, err3 = load_photo(oid2, fname2)
    assert err3 is None and data3
    assert client.delete(f"/records/{r2}", headers=h).status_code == 200


def test_account_freeze_auth_share_422_and_limiter_ready(client, monkeypatch):
    """v0.7.67–0.7.78: account freeze, auth polish, 422 shape, limiter ready, CI."""
    from pathlib import Path
    ...
    assert ready.status_code == 200
    assert ready.json()["db"] == "ok"
    assert ready.json()["limiter"] in ("memory", "redis", "redis_error")

    metrics = client.get("/metrics")
    assert metrics.status_code == 200
    # redis not configured in tests → gauge omitted; limiter label still present
    assert 'limiter=' in metrics.text or "fos_app_info" in metrics.text

    root = Path(__file__).resolve().parents[2]
    account = (root / "mobile" / "src" / "screens" / "AccountScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "BILLING_READONLY_MSG" in account
    assert "billingReadonly" in account
    assert 'title="Cancel request"' in account
    assert "disabled={busy || billingReadonly}" in account

    for name in ("ReportsScreen.tsx", "MyReportScreen.tsx"):
        src = (root / "mobile" / "src" / "screens" / name).read_text(encoding="utf-8")
        assert "onResumeRefresh" in src

    for name in ("CreateScreen.tsx", "TransferScreen.tsx", "InviteScreen.tsx", "PayoutScreen.tsx"):
        src = (root / "mobile" / "src" / "screens" / name).read_text(encoding="utf-8")
        assert "onResumeRefresh" in src and "billingMe" in src

    fmt = (root / "mobile" / "src" / "format.ts").read_text(encoding="utf-8")
    assert "emailFormatError" in fmt and "currencyCodeError" in fmt
    auth = (root / "mobile" / "src" / "screens" / "AuthScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "emailFormatError" in auth and "submitLock" in auth
    invite = (root / "mobile" / "src" / "screens" / "InviteScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert invite.count("Share.share") >= 2
    team = (root / "mobile" / "src" / "screens" / "TeamScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "Share now?" in team or 'text: "Share"' in team

    main = (Path(__file__).resolve().parents[1] / "app" / "main.py").read_text(
        encoding="utf-8"
    )
    assert "RequestValidationError" in main
    assert "normalize_path" in main
    assert "CORS_ORIGINS must list at least one origin" in main
    rl = (
        Path(__file__).resolve().parents[1] / "app" / "services" / "rate_limit.py"
    ).read_text(encoding="utf-8")
    assert "def limiter_health" in rl

    wf = (root / ".github" / "workflows" / "api-tests.yml").read_text(encoding="utf-8")
    assert "health/ready" in wf and "version.py" in wf
    ...
    monkeypatch.setattr(settings, "metrics_token", "tokxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
    monkeypatch.setattr(settings, "cors_origins", "   ")
    try:
        raised = False
        try:
            ...
            raised = True
            assert "CORS_ORIGINS" in str(exc)
            assert "at least one origin" in str(exc)
        assert raised
    finally:
        monkeypatch.setattr(settings, "environment", "development")
        monkeypatch.setattr(settings, "secret_key", "test-secret")
...
def test_detail_freeze_auth_offline_session_csv_ci(client, monkeypatch):
    """v0.7.71–0.7.78: detail freeze, forget auth, expires_in, csv/redis/CI."""
    from datetime import datetime, timedelta, timezone
    from pathlib import Path
    ...
    assert "billingReadonly" in detail
    assert 'title="Add manager note"' in detail
    assert "disabled={busy || billingReadonly}" in detail
    assert 'title="Cancel record"' in detail
    assert "disabled={busy}" in detail

    auth = (root / "mobile" / "src" / "screens" / "AuthScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "Forget remembered" in auth
    assert "storageDelete" in auth
    assert "LAST_SLUG_KEY" in auth and "LAST_EMAIL_KEY" in auth

    api_ts = (root / "mobile" / "src" / "api.ts").read_text(encoding="utf-8")
    assert "OFFLINE_MSG" in api_ts
    assert "Could not reach the server" in api_ts
    app = (root / "mobile" / "App.tsx").read_text(encoding="utf-8")
    assert "OFFLINE_MSG" in app
    assert "expired" in app.lower()

    reports = (
        Path(__file__).resolve().parents[1] / "app" / "routers" / "reports.py"
    ).read_text(encoding="utf-8")
    assert "\\ufeff" in reports or "\ufeff" in reports
    assert "\\x00" in reports or '"\\x00"' in reports or "\\x00" in reports
    assert _csv_text("＝1+1").startswith("'")
    assert "\x00" not in _csv_text("\x00=cmd")
    assert _csv_text("\x00=cmd").startswith("'")

    rl = (
        Path(__file__).resolve().parents[1] / "app" / "services" / "rate_limit.py"
    ).read_text(encoding="utf-8")
    assert "ttl < 0" in rl
    assert "len(key) > 128" in rl

    wf = (root / ".github" / "workflows" / "api-tests.yml").read_text(encoding="utf-8")
    assert "version.py" in wf and '"version"' in wf

    # expires_in on login/register
    owner = _register(client, "flow-0774", "v0774-owner@example.com")
    assert "expires_in" in owner
    assert owner["expires_in"] == int(settings.access_token_expire_minutes) * 60
    h = {"Authorization": f"Bearer {owner['access_token']}"}

    # Create while writable, then freeze for comment/cancel checks
    from app.db import SessionLocal
    from app.models import Organization
    ...
    with SessionLocal() as db:
        org = db.query(Organization).filter(Organization.slug == "flow-0774").one()
        org.billing_status = "past_due"
        db.commit()

    cmt = client.post(f"/records/{rid}/comment", headers=h, json={"note": "mgr note"})
    assert cmt.status_code == 403

    cancel = client.delete(f"/records/{rid}", headers=h)
    assert cancel.status_code == 200, cancel.text

    # logout revokes media JWT — restore writable first for upload
    with SessionLocal() as db:
        org = db.query(Organization).filter(Organization.slug == "flow-0774").one()
    ...
    assert up.status_code == 200, up.text
    photo_url = up.json()["photo_url"]
    media = client.get("/records/media-token", headers=h)
    assert media.status_code == 200
    media_tok = media.json()["access_token"]
    assert client.get(f"{photo_url}?token={media_tok}").status_code == 200
    assert client.post("/auth/logout", headers=h).status_code == 200
    assert client.get("/auth/me", headers=h).status_code == 401
    assert client.get(f"{photo_url}?token={media_tok}").status_code == 401
    ...
    expired_me = client.get("/auth/me", headers={"Authorization": f"Bearer {expired}"})
    assert expired_me.status_code == 401
    assert "expired" in expired_me.json()["detail"].lower()

    # CSV BOM on export
    owner2 = _register(client, "flow-0774b", "v0774b-owner@example.com")
    h2 = {"Authorization": f"Bearer {owner2['access_token']}"}
    exp = client.get("/reports/export.csv", headers=h2)
    assert exp.status_code == 200
    assert exp.text.startswith("\ufeff")

def test_v0775_payout_history_freeze_void_gate_markers():
    """v0.7.75: PayoutHistory billing banner + disable Void + modal re-check."""
    from pathlib import Path

    from app.version import APP_VERSION

    assert APP_VERSION == "0.8.230"
    text = Path(REPO_ROOT / "mobile/src/screens/PayoutHistoryScreen.tsx").read_text(
        encoding="utf-8"
    )
    assert "billingMe" in text
    assert "billingReadonly" in text
    assert "BILLING_READONLY_MSG" in text
    assert "disabled={busy || billingReadonly}" in text
    assert "if (billingReadonly)" in text
    assert "onResumeRefresh" in text


def test_v0776_invite_balances_account_freeze_polish_markers():
...
def test_v0777_export_org_rate_limit_and_access_role_bind_markers():
...
def test_access_token_role_claim_must_match_db_role(client):
    """Forged or missing access JWT role claim is rejected."""
    from datetime import datetime, timedelta, timezone
    ...
    owner = _register(client, "flow-rolebind", "rolebind@example.com")
    user_id = owner["user"]["id"]
    org_id = owner["user"]["organization_id"]
    ver = int(owner["user"].get("token_version") or 0)

    # Sanity: real token still works.
    ok = client.get(
        "/auth/me", headers={"Authorization": f"Bearer {owner['access_token']}"}
    )
    assert ok.status_code == 200

    forged = create_access_token(user_id, org_id, "admin", token_version=ver)
    bad = client.get("/auth/me", headers={"Authorization": f"Bearer {forged}"})
    assert bad.status_code == 401

    # Missing role claim (manually encoded) must also fail.
    ...
    missing = jwt.encode(payload, settings.secret_key, algorithm=settings.algorithm)
    missing_role = client.get(
        "/auth/me", headers={"Authorization": f"Bearer {missing}"}
    )
    assert missing_role.status_code == 401


def test_void_payout_forbidden_when_org_canceled(client):
    """v0.7.75: void settlement blocked under canceled billing freeze."""
    owner = _register(client, "flow-voidcancel", "voidcancel@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    uid = owner["user"]["id"]
    client.post(
        "/records",
    ...
    db = SessionLocal()
    try:
        org = db.query(Organization).filter(Organization.slug == "flow-voidcancel").one()
        org.billing_status = "canceled"
        db.commit()
    finally:
        db.close()

    void = client.post(f"/payouts/{pid}/void", headers=h, json={"note": "undo"})
    assert void.status_code == 403
    detail = void.json()["detail"].lower()
    assert "canceled" in detail or "read-only" in detail or "billing" in detail


def test_v0778_health_cache_control_no_store(client):
    """v0.7.78: live/ready send Cache-Control: no-store."""
    live = client.get("/health/live")
    assert live.status_code == 200
...
def test_v0778_health_cache_control_ci_and_docs_markers():
...
def test_v0779_home_resume_freeze_markers():
    """v0.7.79: Home BILLING_READONLY_MSG + Approve/Team/Balances resume billingMe."""
    from pathlib import Path
...
def test_v0780_confirm_path_freeze_recheck_markers():
...
def test_v0781_expires_in_session_hygiene_markers():
...
def test_v0782_soft_fail_invite_org_ready_media_markers():
...
def test_invite_org_rate_limit_returns_429(client, monkeypatch):
    """Org-scoped invite-org limiter can return 429."""
    from fastapi import HTTPException

    owner = _register(client, "flow-inviteorg", "inviteorg@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}

    calls = {"n": 0}

    def wrap(key, limit=60, window_sec=60):
        if str(key).startswith("invite-org:"):
            calls["n"] += 1
            if calls["n"] > 2:
                raise HTTPException(status_code=429, detail="Rate limit exceeded")
            return None
        # Pass through other invite keys without counting against org cap.
        return None

    monkeypatch.setattr("app.routers.auth.enforce_rate_limit", wrap)
...
def test_v0787_soft_fail_load_more_markers():
    """v0.7.87: load-more/Team reload soft-fail (inline error, no dual Alert)."""
    from pathlib import Path
    ...
    for name, needle in (
        ...
        ("ApproveScreen.tsx", "Load more failed"),
...
def test_v0788_invite_account_settle_freeze_markers():
...
def test_v0789_org_money_rate_limit_markers():
...
def test_payout_org_rate_limit_returns_429(client, monkeypatch):
    """Org-scoped payout-org limiter can return 429 on create."""
    from fastapi import HTTPException

    owner = _register(client, "flow-payorg", "payorg@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    uid = owner["user"]["id"]
    client.post(
        "/records",
    ...
    def wrap(key, limit=60, window_sec=60):
        if str(key).startswith("payout-org:"):
            calls["n"] += 1
            if calls["n"] > 2:
                raise HTTPException(status_code=429, detail="Rate limit exceeded")
            return None
    ...
    for i in range(5):
        last = client.post(
            "/payouts",
            headers={**h, "Idempotency-Key": f"pay-org-{i}"},
            json={"user_id": uid, "kind": "expense_payout", "amount": 1},
        )
        if last.status_code == 429:
            break
    assert last is not None
...
def test_v0790_demo_hygiene_and_ci_prod_docs_markers():
...
def test_v0791_team_approve_confirm_billing_markers():
    """v0.7.91: Team/Approve confirm paths await billingMe."""
    from pathlib import Path
...
def test_v0792_payout_balances_account_confirm_billing_markers():
...
def test_v0793_payout_history_soft_fail_markers():
    ...
    idx = text.find(needle)
    assert idx >= 0
    assert "Alert.alert" not in text[idx : idx + len(needle) + 80]
    assert "Load more failed" in text
    assert 'Alert.alert("Fos", e instanceof Error ? e.message : "Load more failed")' not in text
    assert "await billingMe()" in text


def test_v0794_comment_void_cancel_org_markers():
...
def test_v0795_settle_request_team_payout_void_org_markers():
...
def test_v0796_session_dual_alert_and_soft_retry_markers():
...
def test_v0797_copy_consistency_markers():
...
def test_v0798_decide_org_and_media_nosniff_markers(client):
...
def test_v0799_ci_docs_ready_media_markers():
...
def test_v07100_ready_closeout_freeze_matrix_and_version(client):
    """v0.7.100: version sync; cancel+password OK under freeze; soft-retry contract."""
    from pathlib import Path
    ...
    owner = _register(client, "flow-07100", "v07100@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    pending = client.post(
        "/records",
        headers=h,
    ...
    with SessionLocal() as db:
        org = db.query(Organization).filter(Organization.slug == "flow-07100").one()
        org.billing_status = "past_due"
        db.commit()

    # Cancel pending still allowed under freeze
    cancel = client.delete(f"/records/{rid}", headers={**h, "Idempotency-Key": "c-07100"})
    assert cancel.status_code == 200, cancel.text

    # Password change OK under freeze
    pw = client.post(
        "/auth/password",
        headers=h,
        json={
            "current_password": "secret12",
            "new_password": "secret13",
            "password_confirm": "secret13",
        },
    )
    assert pw.status_code == 200, pw.text
    assert "access_token" in pw.json()
    h2 = {"Authorization": f"Bearer {pw.json()['access_token']}"}

    # Money write blocked; JWT still valid for reads
    blocked = client.post(
        "/records",
        headers={**h2, "Idempotency-Key": "w-07100"},
        json={
            "kind": "expense",
            "amount": 3,
            "category": "Taxi",
    ...
    assert blocked.status_code == 403
    me = client.get("/auth/me", headers=h2)
    assert me.status_code == 200

def test_v07101_payout_soft_fail_and_batch_billing_markers():
    """v0.7.101: Payout boot soft-fail; Pay/Take all await billingMe."""
    from pathlib import Path
    ...
    assert 'text: "Pay all"' in text and "await billingMe()" in text
    assert 'text: "Take all"' in text


def test_v07102_approve_reject_balances_void_billing_markers():
...
def test_v07103_team_reset_confirm_billing_markers():
...
def test_v07108_metrics_cors_ci_markers(client):
...
def test_v07109_auth_inline_form_error_markers():
    ...
    assert "Alert.alert" not in auth  # validation + API failures are inline


def test_v07110_create_photo_account_tg_and_version(client):
    """v0.7.110: quiet photo attach; TG confirm billingMe; version seal."""
    from pathlib import Path

    from app.version import APP_VERSION

    assert APP_VERSION == "0.8.230"
    assert client.get("/health").json()["version"] == "0.8.230"
...
def test_upload_org_rate_limit_returns_429(client, monkeypatch):
    from fastapi import HTTPException

    owner = _register(client, "flow-uporg", "uporg@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    calls = {"n": 0}

    def wrap(key, limit=60, window_sec=60):
        if str(key).startswith("upload-org:"):
            calls["n"] += 1
            if calls["n"] > 2:
                raise HTTPException(status_code=429, detail="Rate limit exceeded")
            return None
    ...
    monkeypatch.setattr("app.routers.media.enforce_rate_limit", wrap)
    last = None
    for i in range(5):
        last = client.post(
            "/media/photo",
            headers={**h, "Idempotency-Key": f"up-{i}"},
            files={"file": ("e.jpg", b"\xff\xd8\xff" + bytes([i]) + b"0" * 64, "image/jpeg")},
        )
        if last.status_code == 429:
            break
    assert last is not None
...
def test_v07111_freeze_confirm_photo_org_and_copy_markers():
...
def test_v07112_session_expired_storage_markers():
    ...
    assert "await storageSet(TOKEN_EXP_KEY, String(expMs));" in api
    # expiry metadata is written before the JWT
    save = api.find("export async function saveToken")
    assert save >= 0
    chunk = api[save : save + 900]
    assert chunk.index("TOKEN_EXP_KEY") < chunk.index("TOKEN_KEY, token")


def test_v07113_stale_list_soft_fail_markers():
    """v0.7.113: list reloads retain rows on refresh failure."""
    from pathlib import Path
    ...
    for name in ("HomeScreen.tsx", "ApproveScreen.tsx", "LedgerScreen.tsx", "PayoutHistoryScreen.tsx"):
        ...
        idx = text.find(needle)
        assert idx >= 0
        window = text[max(0, idx - 160) : idx + 80]
        assert "setRows([])" not in window


def test_v07114_pane_isolate_balances_account_markers():
...
def test_v07115_reference_data_preserve_markers():
    ...
    assert 'setOrgSlug("")' not in invite[ls:ls_end]


def test_v07116_resume_settlement_context_markers():
...
def test_v07117_auth_api_errors_inline_markers():
...
def test_v07118_mutation_org_limits_markers_and_429(client, monkeypatch):
    """v0.7.118: record-write-org / org-update-org / settle-cancel-org."""
    from pathlib import Path
    ...
    owner = _register(client, "flow-rwrite", "rwrite@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    calls = {"n": 0}

    def wrap(key, limit=60, window_sec=60):
        if str(key).startswith("record-write-org:"):
            calls["n"] += 1
            if calls["n"] > 2:
                raise HTTPException(status_code=429, detail="Rate limit exceeded")
            return None
    ...
    for i in range(5):
        last = client.post(
            "/records",
            headers={**h, "Idempotency-Key": f"rw-{i}"},
            json={
                "kind": "expense",
                "amount": 10 + i,
                "category": "Office",
...
def test_v07119_read_org_limits_markers_and_429(client, monkeypatch):
    """v0.7.119: balances-read-org / reports-read-org / finance-list-org."""
    from pathlib import Path
    ...
    owner = _register(client, "flow-balread", "balread@example.com")
    h = {"Authorization": f"Bearer {owner['access_token']}"}
    calls = {"n": 0}

    def wrap(key, limit=60, window_sec=60):
        if str(key).startswith("balances-read-org:"):
            calls["n"] += 1
            if calls["n"] > 2:
                raise HTTPException(status_code=429, detail="Rate limit exceeded")
            return None
    ...
    last = None
    for _ in range(5):
        last = client.get("/records/balance/team", headers=h)
        if last.status_code == 429:
            break
    assert last is not None
    assert last.status_code == 429
...
def test_v07120_early_error_headers_docs_ci_version_seal(client):
    """v0.7.120: early 413 header parity; CI lock sync; version seal."""
    from pathlib import Path
    ...
    fn = main.find("def _security_headers")
    assert fn >= 0
    block = main[fn : fn + 700]
    assert "Permissions-Policy" in block
    assert "Cross-Origin-Opener-Policy" in block
    assert "Cache-Control" in block
    assert "def _json_error" in main
    ...
    assert "no-store" in (huge.headers.get("cache-control") or "").lower()
    assert "nosniff" in (huge.headers.get("x-content-type-options") or "").lower()
    assert "geolocation=()" in (huge.headers.get("permissions-policy") or "")
    assert (huge.headers.get("cross-origin-opener-policy") or "") == "same-origin"
...
def test_v07181_190_hsts_early_error_and_metrics_tokens(client, monkeypatch):
    ...
    metrics_tok = "metrics-secretxxxxxxxxxxxxxxxxxx"
    assert _tok_ok(metrics_tok, metrics_tok)
    assert not _tok_ok("short", metrics_tok)
    monkeypatch.setattr(settings, "metrics_token", metrics_tok)
    monkeypatch.setattr(settings, "rate_limit_enabled", True)
    reset_limiter_for_tests()
    ...
    assert (
        client.get(
            "/metrics", headers={"X-Metrics-Token": metrics_tok}
        ).status_code
        == 200
    )
    assert (
        client.get(
            "/metrics", headers={"Authorization": f"Bearer {metrics_tok}"}
        ).status_code
        == 200
    )
    reset_limiter_for_tests()
...
@pytest.mark.parametrize(
    ("key_prefix", "method", "path", "payload"),
    [
        (
            "password-org:",
            "POST",
        ...
        ("logout-org:", "POST", "/auth/logout", None),
        ("billing-me-org:", "GET", "/billing/me", None),
        ("categories-org:", "GET", "/records/categories", None),
        ("media-token-org:", "GET", "/records/media-token", None),
    ],
)
def test_v07201_new_org_keys_markers_and_param_429(
    client, monkeypatch, key_prefix, method, path, payload
):
    from fastapi import HTTPException

    router_keys = {
        "auth.py": ("password-org:", "logout-org:", "accept-invite-org:"),
        "billing.py": ("billing-me-org:",),
    ...
    slug_part = key_prefix.removesuffix(":")
    owner = _register(
        client,
        f"seal-201-{slug_part}",
        f"seal-201-{slug_part}@example.com",
    )
    headers = {"Authorization": f"Bearer {owner['access_token']}"}
    seen = []

    def block_selected(key, limit=60, window_sec=60):
        seen.append(str(key))
        if str(key).startswith(key_prefix):
            raise HTTPException(status_code=429, detail="Rate limit exceeded")

    # Most routers import at call time; auth also keeps a module-level reference.
    monkeypatch.setattr("app.services.rate_limit.enforce_rate_limit", block_selected)
    monkeypatch.setattr("app.routers.auth.enforce_rate_limit", block_selected)
    response = client.request(method, path, headers=headers, json=payload)
    assert response.status_code == 429, response.text
    assert any(key.startswith(key_prefix) for key in seen)


def test_v07202_org_limit_aggregates_across_users(client, monkeypatch):
    from fastapi import HTTPException

    owner = _register(client, "seal-202-shared", "seal-202-owner@example.com")
    owner_headers = {"Authorization": f"Bearer {owner['access_token']}"}
    invited = client.post(
        "/orgs/invite",
        headers=owner_headers,
        json={
            "email": "seal-202-employee@example.com",
            "full_name": "Shared Budget Employee",
            "role": "employee",
            "password": "secret12",
            "password_confirm": "secret12",
        },
    ...
    login = client.post(
        ...
        json={
            "email": "seal-202-employee@example.com",
            "password": "secret12",
            "organization_slug": "seal-202-shared",
        },
    )
    assert login.status_code == 200, login.text
    employee_headers = {
    ...
    def shared_budget(key, limit=60, window_sec=60):
        if str(key).startswith("billing-me-org:"):
            org_hits.append(str(key))
            if len(org_hits) > 2:
                raise HTTPException(status_code=429, detail="Rate limit exceeded")

    monkeypatch.setattr("app.services.rate_limit.enforce_rate_limit", shared_budget)
    responses = [
        client.get("/billing/me", headers=owner_headers),
        client.get("/billing/me", headers=employee_headers),
        client.get("/billing/me", headers=owner_headers),
    ]
    assert [response.status_code for response in responses] == [200, 200, 429]
    assert len(set(org_hits)) == 1
...
def test_v07203_accept_invite_org_limit_markers():
    ...
    resolved = block.index("found = find_user_by_invite_token")
    org_limit = block.index('f"accept-invite-org:')
    assert resolved < org_limit
    assert "accept-invite-org:" not in block[:resolved]
...
def test_v07204_non_money_idempotency_markers():
    ...
    for source in (auth, team, billing):
        ...
        assert "store_idem" in source
        assert "require_idem_match" in source
    for scope in ("orgs.update", "orgs.invite", "auth.accept_invite"):
        assert f'scope="{scope}"' in auth
    for scope in (
        "team.member_active",
        "team.member_role",
        "team.member_password",
        "team.member_reset_token",
    ):
        assert f'scope="{scope}"' in team
    for scope in (
        "billing.set_plan",
        "billing.telegram_chat",
        "billing.telegram_test",
    ):
        assert f'scope="{scope}"' in billing
...
def test_v07205_idempotency_mismatch_marker():
    ...
    # Behavioral 409 coverage is in test_v07131_140_auth_team_and_org_idempotency.
    suite = Path(__file__).read_text(encoding="utf-8")
    assert "test_v07131_140_auth_team_and_org_idempotency" in suite
    assert 'json={"name": "Other Company", "currency": "IDR"}' in suite


def test_v07206_security_header_matrix(client):
    responses = [
        ...
        (
            client.post(
                "/auth/login",
                json={
                    "email": "seal-206-missing@example.com",
                    "password": "secret12",
                    "organization_slug": "seal-206-missing",
                },
            ),
            401,
        ),
...
def test_v07207_production_config_rejection_matrix(monkeypatch):
    from app.config import settings
    from app.main import _validate_runtime_settings
    ...
    baseline = {
        ...
        "database_url": (
            "postgresql+psycopg2://fos:fos@db.example.com:5432/"
            "fos?sslmode=require"
        ),
        "db_pool_size": 5,
        "db_max_overflow": 10,
        "metrics_token": "seal-207-metrics-token-at-least-32",
        "smtp_host": "",
        "smtp_use_tls": True,
    }
    cases = [
        ...
        ({"enable_hsts": True, "hsts_max_age": 0}, "HSTS_MAX_AGE"),
        ({"s3_endpoint_url": "http://s3.example.com"}, "S3_ENDPOINT_URL"),
    ]
...
def test_v07208_session_skip_and_listutil_markers():
    ...
    assert api.is_file() and "shouldSkipErrorAlert" in api.read_text(encoding="utf-8")
    list_source = list_util.read_text(encoding="utf-8")
    assert list_util.is_file()
    assert "mergeById" in list_source
    assert "hasMorePage" in list_source
...
def test_v07210_format_utc_and_listutil_node():
    ...
    assert "mobile unit smoke ok" in result.stdout
...
def test_v07220_version_seal(client):
    ...
    assert response.json()["version"] == "0.8.230"
